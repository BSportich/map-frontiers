...
ViewEvaluator::ViewEvaluator(const voxblox_map::VoxbloxMap& map, const std::string& method_name, const SensorModel& sensor_lidar, double threshold, int radius_max_surface) 
{
  ...
  int vs = m_map.getVoxelSize() ;  // voxel temp variable
  //Frontiers data 
  m_frontier6 = false;
  if(m_frontier6 == true){
...
//bool ViewEvaluator::isSurfaceFrontier_TSDF(const Eigen::Vector3d& voxel){
  ...
  for (int i = 0; i < 26; ++i) {
    ...
    if (voxel_state == voxblox_map::VoxbloxMap::UNKNOWN) {
      ...
      ROS_INFO( "FOUND UNKNOWN %f %f %f", (voxel + c_neighbor_voxels_[i]).x(), (voxel + c_neighbor_voxels_[i]).y(), (voxel + c_neighbor_voxels_[i]).z());
      return true;
    }
